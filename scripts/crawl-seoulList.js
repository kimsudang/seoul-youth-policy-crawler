...
const BASE_URL =
  'https://youth.seoul.go.kr/infoData/plcyInfo/ctList.do?sprtInfoId=&plcyBizId=&key=2309150002&sc_detailAt=&orderBy=regYmd+desc&blueWorksYn=N&tabKind=002&sw=';

export default async function crawlSeoulList() {
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  console.log('🌀 [서울시정책] 마지막 페이지 번호 가져오는 중...');
  ...
  console.log(`✅ [서울시정책] 마지막 페이지: ${lastPage} 페이지`);

  const results = [];

  for (let pageIndex = lastPage; pageIndex >= 1; pageIndex--) {
    console.log(`📄 [서울시정책] ${pageIndex} 페이지 크롤링 중...`);
    ...
    try {
      ...
      console.log(`⚠️ [서울시정책] ${pageIndex} 페이지에 리스트 없음 (건너뜀)`);
      continue;
    }
    ...
    console.log(`✅ [서울시정책] ${pageIndex} 페이지에서 ${pageData.length}개 수집됨`);

    results.push(...pageData);
  }

  // ✅ 중복 제거
  console.log(`🗂️ [서울시정책] 중복 제거 전: ${results.length}개`);
